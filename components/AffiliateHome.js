...
import AffiliatePixel from "./AffiliatePixel";
...
function buildTrackedHref(affiliateSlug, funnelSlug) {
  const base = `/affiliate/${affiliateSlug}/${funnelSlug}`;
  if (typeof window === "undefined") return base;

  const params = new URLSearchParams(window.location.search);
  const allowedKeys = [
    "aff", "affiliate_id", "sub1", "sub2",
    "click_id", "clickid",
...
export default function AffiliateHome({ affiliateSlug, affiliate }) {
  return (
      ...
      <section id="services" className="rw-services">
        ...
        <div className="rw-services-grid">
          {services.map((item) => (
            <Link
              key={item.slug}
              href={buildTrackedHref(affiliateSlug, item.slug)}
